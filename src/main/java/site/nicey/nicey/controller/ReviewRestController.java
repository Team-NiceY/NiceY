...
//	}
	@GetMapping("/board/{id}")
	@ApiOperation(value="해당 비디오의 리뷰 전체를 가져온다")
	public ResponseEntity<?> list(@PathVariable int id){
		List<Board> list = reviewService.getList(id); //전체 조회
		if(list == null || list.size() == 0)
			return new ResponseEntity<Void>(HttpStatus.NO_CONTENT);
		return new ResponseEntity<List<Board>>(list, HttpStatus.OK);
	}
...
//	}
	...
	public ResponseEntity<Board> write(Board board){
		reviewService.writeBoard(board);
	...
	public ResponseEntity<Void> update(@RequestBody Board board){
		reviewService.modifyBoard(board);
		//위와같은 상황 대비
